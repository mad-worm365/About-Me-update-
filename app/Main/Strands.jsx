...
void main() {
  ...
  // Vertical mode: strands flow top-to-bottom instead of left-to-right
  if (uVertical > 0.5) {
    uv = vec2(uv.y, -uv.x);
  }
  ...
  // Soft viewport fade so the canvas edge never reads as a hard box
  vec2 edgeUv = gl_FragCoord.xy / uResolution;
  float edgeFade =
      smoothstep(0.0, 0.12, edgeUv.x) *
      smoothstep(0.0, 0.12, edgeUv.y) *
      smoothstep(0.0, 0.12, 1.0 - edgeUv.x) *
      smoothstep(0.0, 0.12, 1.0 - edgeUv.y);
  col *= edgeFade;
  alpha *= edgeFade;
...
export default function Strands({
  ...
  useEffect(() => {
    ...
    const update = (t) => {
      ...
      program.uniforms.uVertical.value = current.vertical ? 1 : 0;
